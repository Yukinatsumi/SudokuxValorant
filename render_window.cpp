...
//Tạo window và lưu kích thước window, tạo renderer.
...
void RenderWindow::loadTextures(const char* filePath, vector<SDL_Texture*>& o_list)
{
    ...
    {
        // tải texture và lưu trong vector o_list
        o_list.push_back(loadTexture(currentTexture.c_str()));
    }
}

// Kiểm tra FPS của màn hình
...
// Lấy thông tin hình ảnh và vẽ hình ảnh bằng renderer
...
// Vẽ màu từng ô và viền bảng Sudoku
void RenderWindow::render(Board& board)
{
    for(Square& s : board.getSquares()) render(s);
    for(int i = 0; i < 3; i++)
    {
        for(int j = 0; j < 3; j++) renderBox(Vector2f(board.getStartPoint().x + j * 3 * board.getSquareSize(), board.getStartPoint().y + i * 3 * board.getSquareSize()), 3 * board.getSquareSize(), 3 * board.getSquareSize());
    }
}
